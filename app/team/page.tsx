        ...
        <section className="grid gap-8 md:grid-cols-2">
          {[
            ...
            <div key={member.name} className="card p-6 text-center">
              <div className="mx-auto mb-4 flex h-28 w-28 items-center justify-center rounded-full bg-[color:var(--kp-fog)] text-3xl text-primary">
                {member.initials}
              </div>
        ...
        <section className="text-center space-y-4">
          ...
          <div className="grid gap-3 md:grid-cols-3">
            {['Full-stack Engineers (Rust/React)', 'Systems Ecologists', 'Pilot Coordinators'].map((role) => (
              <div key={role} className="card p-4 text-sm font-semibold text-primary shadow-none border-[color:var(--kp-fog)]">
                {role}
              </div>
            ))}
          </div>
